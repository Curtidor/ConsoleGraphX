...
{
	Dispatcher<Entity*>::RegisterListener("RemoveComponentstruct Camera", _CameraRemovedHandler);
	Camera* cam = new Camera(Screen::GetWidth_A(), Screen::GetHeight_A(), Vector3());
...
void CameraSystem::SetCameraState(Camera* cam, bool state)
{
	...
	if (itCam == _s_cameras.end())
	{
		Debugger::S_LogMessage("When using set camera state, the camera must be registered already");
		return;
	}
...
const std::unordered_set<Camera*> CameraSystem::GetActiveCameras()
{
	...
	{
		Debugger::S_LogMessage("[CAMERA SYSTEM], using backup camera!");
